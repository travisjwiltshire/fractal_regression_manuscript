...
+---------------+-----------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `mc_ARFIMA()` | Simulate various types of biariate correlated noise processes.                                                                    | Returns two vectors of length `N` according to the specified noise `process` and parameters                                                                                      |
+---------------+-----------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Then, we can run DFA on these using the example code below. Note that this example uses a linear detrending with minimum scale (`sc_min`) of 16, a maximum scale (`sc_max`) of 1/4 the time series length, and logarithmically spaced scale factor (`scale_ratio`) of 2.

```{r, echo = TRUE}
dfa.white <- dfa(x = white.noise, order = 1, verbose = 1,
                sc_min = 16, sc_max = length(pink.noise)/4, scale_ratio = 2)
...
In terms of output from the above examples, for white noise, we observed that $\alpha$ = `r dfa.white$alpha`, for pink noise we observed that $\alpha$ = `r dfa.pink$alpha`, and since we simulated anti-correlated noise with H = 0.25, we observed a close estimate of the $\alpha$ = `r dfa.anti.corr$alpha`. In terms of the objects saved from the `dfa()` function, one commonly inspects the `logScales`-`logRms` plots. Given the estimates above, we see in Figure 1 that the slopes for white noise, pink noise, and anti-correlated noise conform to our expectations.

**Figure 1**

*LogScale-LogFluctuation plots for white noise (left), pink noise (middle), and anti-correlated noise (right)*

```{r echo=FALSE, warning=FALSE}
dfa.white.df <- as.data.frame(cbind(dfa.white$logScales,dfa.white$logRms))

names(dfa.white.df) <- c("logScales", "logRms")

dfa.pink.df <- as.data.frame(cbind(dfa.pink$logScales,dfa.pink$logRms))

names(dfa.pink.df) <- c("logScales", "logRms")

dfa.anti.corr.df <- as.data.frame(cbind(dfa.anti.corr$logScales,dfa.anti.corr$logRms))

names(dfa.anti.corr.df) <- c("logScales", "logRms")

# Create the plot

p1 <- ggplot(data=dfa.white.df, aes(x = logScales, y = logRms)) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("White noise") + theme(plot.title = element_text(hjust = 0.5)) +

annotate("text", x=5, y = 2, parse = TRUE, label = "alpha == 0.52") +

ylim(-1,6)

p2 <- ggplot(data=dfa.pink.df, aes(x = logScales, y = logRms)) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("Pink noise") + theme(plot.title = element_text(hjust = 0.5)) +

annotate("text", x=5, y = 2, parse = TRUE, label = "alpha == 0.91") +

ylim(-1,6)

p3 <- ggplot(data=dfa.anti.corr.df, aes(x = logScales, y = logRms)) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("Anti-correlated noise") + theme(plot.title = element_text(hjust = 0.5)) +

annotate("text", x=5, y = 2, parse = TRUE, label = "alpha == 0.26")+
...
For an empirical example, we apply the `dfa()` function to DESCRIBE DATA HERE. Movement Data. Importantly, regarding the question one can ask using DFA, we observe that ANSWER TO QUESTION HERE.

**Figure 2**

*LogScale-LogFluctuation plots for empirical time series (ADD ME)*
...
To demonstrate the use of `mfdfa()`, we work with data included in our package (`fractaldata`), that was originally provided by @ihlenIntroductionMultifractalDetrended2012 . It includes a white noise time series, monofractal time series, and a multifractal time series. Investigation of the properties could also examine simulated multifractal Brownian motion and multifractal Gaussian noise.
...
General recommendations for choosing the min and max scale are an sc_min = 10 and sc_max = (N/4), where N is the number of observations. See Eke et al. (2002) [@ekeFractalCharacterizationComplexity2002] and Gulich and Zunino (2014) [@gulichCriterionDeterminationOptimal2014] for additional considerations.
...
-   Practical consideration of bivariate methods
    ...
    -   Likens et al. 2019 found positive bias of linear and quadratic trends on MRA beta estimates at larger scales that could be mitigated with larger detrending order. This involves checking teh time series with time as a predictor and polynomials.

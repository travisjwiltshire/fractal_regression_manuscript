...
We demonstrate the use of MLRA on a simple two-element system defined by:
...
In the above system, $y(t)$ is coupled to $x(t)$ at lag 50 but $x(t)$ is not coupled to $y(t)$. Running `mlra()` is the same as `mra()` with one additional parameter: `lags`. In Figure 14, we see the output of MLRA when regressing $y(t)$ on $x(t)$ with a maximum lag of 100 time points. Figure 14 clearly represents the expected results. There is a strong positive relationship $\beta(s,l)$ at lag 50 that clearly approaches 1.0. At other lags however, the values are close to zero.

**Figure 14**

*MLRA output for simulated simple two-element system regressing* $y(t)$ on $x(t)$
...
for (t in 51:length(x)){
  ...
  x[t] = 1 + 0.0*y[t-50] + 0.0*x[t-50] + e1[t]
}
x = x[51:length(x)]
y = y[51:length(y)]
...
In terms of the empirical data, below we apply MLRA to the COPxy data for the firm and the foam examples. Here we also investigate up to a lag of 100. NEED TO DECIDE ON ORDER HERE FOR MLRA AND ADD SUMMARY OF PLOTS/ANSWER TO QUESTION.
...
scales <- logscale(scale_min = 16, scale_max = length(open_firm_copx_diff)/4, scale_ratio = 1.1)
mlra.firm.out <- mlra(scale(open_firm_copy_diff), scale(open_firm_copx_diff), order = 1, scales = scales, lags = 1000, direction = 'p')
x <- 0:1000
y <- scales
fields::image.plot(x, y, mlra.firm.out$betas, axes=TRUE, legend.lab = "Beta Coefficient", ylab="Scale", xlab="Lag", main="MLRA on COPxy firm",zlim=c(-10,10))

scales <- logscale(scale_min = 16, scale_max = length(open_foam_copx_diff)/4, scale_ratio = 1.1)
mlra.foam.out <- mlra(scale(open_foam_copy_diff), scale(open_foam_copx_diff),order = 1, scales = scales, lags = 1000, direction = 'p')
x <- 0:1000
y <- scales
fields::image.plot(x, y, mlra.foam.out$betas, axes=TRUE, legend.lab = "Beta Coefficient", ylab="Scale", xlab="Lag", main="MLRA on COPxy foam",zlim=c(-10,10))

min(mlra.foam.out$betas)
max(mlra.foam.out$betas)
min(mlra.firm.out$betas)
max(mlra.firm.out$betas)

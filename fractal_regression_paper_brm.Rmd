...
scales [@kello2010]. These regularities often follow a power-law (i.e.,
...
indicator of system health [@goldbergerFractalDynamicsPhysiology2002],
adaptability [@bakSelforganizedCriticalityExplanation1987], control
...
[@hausdorffFractalDynamicsHuman1996, @delignières2009], limb movements
...
@eulerWorkingMemoryPerformance2016]. And, beyond an individual level,
the methods have been used to study human-machine system interaction
[@likensExperimentalControlScaling2015], tool use
[@kelty-stephenMultifractalTemporalCorrelations2016, @favela2021], and
...
Correlation (DCCA) [@zebendeDCCACrosscorrelationCoefficient2011;
...
insights in important areas such as healthcare [@cavanaugh2017] and
education [@snow2016]. In this work, we provide an overview of our
`fractalRegression` package, provide simulated and empirical examples of
it's functions, and provide practical advice on the successful
application of these methods.

*NEED THIS: Connect better to regression part (see 2019 paper from Aaron
and Nia Multiscale regression intro and discussion)*

# Package Overview

Our `fractalRegression` package for R
[@rcoreteamLanguageEnvironmentStatistical2018] is built on a C++
...
have been implemented in the 'fracLab' library for MATLAB [@legrand2003]
and other toolboxes that are mainly targeted at multifractal analysis
[@ihlen2010; @ihlenIntroductionMultifractalDetrended2012]. In terms of
open access packages, there are other packages that implement some, but
not all of the same functions such as the `fathon` package
[@bianchi2020] that has been implemented in Python as well as the R
packages: `fractal` [defunct], `nonlinearTseries`
...
Our `fractalRegression` package is unique in this combination of
...
[@kelty-stephenMultifractalTemporalCorrelations2016], we provide a brief
...
`fARMA` R package. It only requires the number of observations `n`, and
the Hurst exponent `H`. In particular, we simulate white noise, pink
...
function, one commonly inspects the log_scales-log_rms plots. Given the
...
each respective line, and are generated using the `dfa.plot()` function.

**Figure 1**

*Log scale-Log fluctuation plots for white noise (top), pink noise
...
[@santos2016]. For the empirical examples, we use two different time
...
interesting fractal dynamics [@collins1993 @delignières2011
...
*Log scale-Log fluctuation plots for empirical difference COPx time
...
these time series; a phenomenon known as crossovers points [@collins1993
...
@kelty-stephenMultifractalTemporalCorrelations2016 Figure 3 for a
...
*Time series from Ihlen (2012) corresponding to white noise,
...
shown in the example below, one needs to enter the time series `x` to
perform the analysis on, the range of `q` order exponents to use, the
`order` of polynomial detrending, and the `scales` for the analysis. In
this case, we define our `scales` by choosing logarithmically spaced
...
logarithm bases. We provide the `logscale` function to facilitate scale
construction.
...
*mfdfa.plots for mono-(top) and multifractal series (bottom). The four
...
`r max(multi.mf.dfa.out$h) - min(multi.mf.dfa.out$h)`. If plot the
...
out parameters appropriate for the data and run `mfdfa()`.
...
scale-log fluctuation plots [@pengMosaicOrganizationDNA1994;
@perakakis2009]. Cross-over points (or a visible change in the slope as
...
[@kantelhardtDetectingLongrangeCorrelations2001] to determine if a
...
[@ekeFractalCharacterizationComplexity2002] and Gulich and Zunino (2014)
...
@zebendeDCCACrosscorrelationCoefficient2011 ) is a bivariate extension
...
asked with it are: a) *How does correlation between two time series
...
derives from originating literature[@kristoufek2015 @likens2019]. Thus,
...
*Subset of two time series exhibiting long range correlation and long
...
the `dcca()` on these time series, we use the code below, where we first
...
*DCCA output for long range correlation and long range
cross-correlation*
...
additional plot and analysis features of `dcca()` by modifying the above
code as shown below `ci = TRUE`. Loess smoothing can also be applied to
both $\rho(s)$ and its confidence intervals using `loess.rho = TRUE` and
`loess.ci = TRUE`. Those latter options are useful for reducing the
impact of increasing variance in estimates of $\rho(s)$ at large scales
[@likens2019]. Note though that a much larger set of calculations takes
...
*DCCA output for long range correlation and short range
cross-correlation*
...
set appropriate values for `scales` and apply the `dcca()` function to
...
*DCCA output for empirical COPx and COPy balance data for the eyes open
...
`r max(dcca.out.open.firm$rho)`' compared to
`r max(dcca.out.open.foam$rho)` for the foam condition. Second, we
observe for the foam example, that the time scale of maximum correlation
is `r dcca.out.open.firm$scales[which.max(dcca.out.open.firm$rho)]`,
which is a larger time scale when compared to the foam example, which
had a maximum correlation at scale r
`dcca.out.open.foam$scales[which.max(dcca.out.open.foam$rho)]`. Third,
...
asymmetric $\beta$ coefficients are estimated (see @likens2019;
...
*MRA output for long range correlation and long range
cross-correlation.*
...
$\beta$ = `r max(mra.firm.out.yx$betas)`). Notice as well how Figure \#
...
*MRA output for balance data on foam surface with COPx predicting COPy
...
a) *How does the influence of one time series on another time series
change as a function of scale at different time lags?* and b) *Does the
...
not coupled to $y(t)$. Running `mlra()` is the same as `mra` with one
additional parameter: `lags`. In Figure #, we see the output of MLRA
...
*MLRA output for simulated simple two-element system regressing* $y(t)$
on $x(t)$*.*
...
*MLRA output for balance data on foam surface with COPx predicting COPy
...
exist [@kantelhardtMultifractalDetrendedFluctuation2002]. Note that
...
equally sized sample of the surrogate series (see @kantz2003
@moulder2018 @wiltshire2019 for examples).
...
their implementation. This `fractalRegression` package is our effort to

...
scales <- ifultools::logScale(scale.min = 16, scale.max = 1025, scale.ratio = 2)
```
...
In terms of output from the above examples, for white noise, we observed that $\alpha$ = `r dfa.white$alpha`, for pink noise we observed that $\alpha$ = `r dfa.pink$alpha`, and since we simulated anti-correlated noise with H = 0.25, we observed a close estimate of the $\alpha$ = `r dfa.anti.corr$alpha`. In terms of the objects saved from the `dfa()` function, one commonly inspects the `log_scales`-`log_rms` plots. Given the estimates above, we see in Figure 1 that the slopes for white noise, pink noise, and anti-correlated noise conform to our expectations.

**Figure 1**

*Log scale-Log fluctuation plots for white noise (left), pink noise (middle), and anti-correlated noise (right)*

```{r echo=FALSE, warning=FALSE}
dfa.white.df <- as.data.frame(cbind(dfa.white$log_scales,dfa.white$log_rms))

names(dfa.white.df) <- c("log_scales", "log_rms")

dfa.pink.df <- as.data.frame(cbind(dfa.pink$log_scales,dfa.pink$log_rms))

names(dfa.pink.df) <- c("log_scales", "log_rms")

dfa.anti.corr.df <- as.data.frame(cbind(dfa.anti.corr$log_scales,dfa.anti.corr$log_rms))

names(dfa.anti.corr.df) <- c("log_scales", "log_rms")

# Create the plot

p1 <- ggplot(data=dfa.white.df, aes(x = log_scales, y = log_rms) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("White noise") + theme(plot.title = element_text(hjust = 0.5)) + annotate("text", x=5, y = 2, parse = TRUE, label = "alpha == 0.54") +

ylim(-1,6)

p2 <- ggplot(data=dfa.pink.df, aes(x = log_scales, y = log_rms)) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("Pink noise") + theme(plot.title = element_text(hjust = 0.5)) + 
annotate("text", x=5, y = 2, parse = TRUE, label = "alpha == 0.92") +

ylim(-1,6)

p3 <- ggplot(data=dfa.anti.corr.df, aes(x = log_scales, y = log_rms)) + geom_point() + geom_smooth(method='lm', formula = y~x, color="black") + ggtitle("Anti-correlated noise") + theme(plot.title = element_text(hjust = 0.5)) +
...
scales <- ifultools::logScale(scale.min = 16,scale.max = 1024,scale.ratio = 1.1)
...
A common way to understand if there is evidence of multifractality is to examine a plot showing the slopes of the `log2Fq` at the `log2Scale` values. If all the plots have the same slope, that provides evidence of monofractality. If there are distinct slopes, then there is evidence of multifractality. It's also important to check here that the slopes of `log2Scale` and `log2Fq` are largely linear, thus implying that they are scale invariant. If not, then it could be the case that a higher order polynomial detrending is appropriate (see Kantelhardt et al., 2001). Figure 4 shows what we would expect for a monofractal and multifractal signal. In other words, the monofractal signal shows a consistent slope, whereas the multifractal signal shows variability in the slopes.

**Figure 4**

*Estimates of log Fq for across log scales for mono- and multifractal series.*
...
A common metric for comparing the multifractal spectrum is to calculate the width ($W$) as $h_{max} - h_{min}$. Let's do this to compare the monofractal and multifractal time series. We observe in this case that for the monofractal signal $W_{mono} =$ `r max(mono.mf.dfa.out$h) - min(mono.mf.dfa.out$h)` and $W_{multi} =$ `r max(multi.mf.dfa.out$h) - min(multi.mf.dfa.out$h).` If plot the multifractal spectra D(h) against h, we clearly observe the difference in the widths of the multifractal spectra for the mono- and multifractal signals, as shown in Figure 5 below.

**Figure 5**

*Multifractal spectra D(h) and h for monofractal and multifractal signals.*

```{r, echo=FALSE}
plot(mono.mf.dfa.out$h,mono.mf.dfa.out$Dh, type='b', lwd=1, lty=2, pch=19,ylim=c(-0.4,1.2),xlim=c(-.8,2), ylab="D(h)", xlab="h", main= "Multifractal Spectra")
lines(multi.mf.dfa.out$h,multi.mf.dfa.out$Dh, type='b', pch=19,lwd=3, col="blue")
legend(-.85,1.2, legend = c("Monofractal", "Multifractal"), col=c("black", "blue"), lwd=3)
```

Simulated data: mfbrownian motion from Ihlen matlab (Aaron might have R port) see mbm_mgn for R from aaron - Empirical data: EPICLE Movement Data?Investigation of the properties could also examine simulated multifractal Brownian motion and multifractal Gaussian noise.

#### DFA and MFDFA Considerations

We recommend a few points of consideration here in using this function. One is to be sure to evaluate whether there are cross-over points in the logScale-logFluctuation plots [@pengMosaicOrganizationDNA1994; @perakakis2009]. Cross-over points (or a visible change in the slope as a function of scale) indicate that a mono-fractal characterization does not sufficiently characterize the data. If cross-over points are evident, we recommend proceeding to using the `mfdfa()` to estimate the multifractal fluctuation dynamics across scales.
...
Assuming we were using IAAFFT to compare the multifractal width ($W$) between the observed signal and the surrogate signals, recall that the observed was $W_{multi} =$ `r max(multi.mf.dfa.out$h) - min(multi.mf.dfa.out$h)`. Now, we can calculated the average multifractal width across all of the generated surrogates and we observe that $W_{surr} =$ `r mean(mf.width`), which is narrower than the spectrum from the multifractal signal.

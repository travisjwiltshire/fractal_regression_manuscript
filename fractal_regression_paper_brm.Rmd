...
For an empirical example, we apply the `dfa()` function to the Human Balance Dataset (CITE). DESCRIBE DATA HERE. We chose this dataset because postural sway data are known to exhibit interesting fractal dynamics (CITES) and we can systematically evaluated the data for all of the univariate and bivariate analyses detailed in this work. Importantly, regarding the question one can ask using DFA, we observe that ANSWER TO QUESTION HERE.

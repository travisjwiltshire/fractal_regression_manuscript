...
`fARMA` R package. It requires the number of observations `n`, and the
Hurst exponent `H`. In particular, we simulate white noise, pink noise,
...
*Log scale-Log fluctuation plots for empirical differenced COPx time
...
the slopes. Importantly, Figure 5, using our `mfdfa.plot()` function
...
additional plot and analysis feature of `dcca()` by modifying the above
...
observations although larger is better [@delignieres2006]. If multiple
...
al. (2002) [@ekeFractalCharacterizationComplexity2002] and Gulich and
...
have other criteria [@damouras2010 @marmelat2019].
...
mitigated with a larger detrending order [@likens2019]. Therefore it is
...
lag using a critical value that is dependent on time scales [@shen2015].
...
In this paper, we advance the `fractalRegression` R package and provide

...
(MFDFA) [@kantelhardtMultifractalDetrendedFluctuation2002], Detrended
Cross Correlation (DCCA) [@zebendeDCCACrosscorrelationCoefficient2011;
...
[@kelty-stephenMultifractalTemporalCorrelations2016]; so we provide a
...
a range of exponents of the qth order. The key question that can be
answered by MFDFA is: *how does the magnitude and direction of long
range correlation change over time within a single time series?*
...
brings the analyses into a predictive framework (CITE). The key
questions that can be answered by it are: a) *How does the influence of
one time series on another time series change as a function of scale?*
and b) *What is/are the dominant (time) scale(s) of influence of one
time series on another time series?*

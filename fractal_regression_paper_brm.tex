...
the \texttt{fgn\_sim} function. In particular, we simulate white noise, pink
noise, and anti-correlated fractional Gaussian noise.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Generate pink noise}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{34534534}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Run DFA on pink noise}
\NormalTok{dfa.pink.out }\OtherTok{\textless{}{-}} \FunctionTok{dfa}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pink.noise, }\AttributeTok{order =} \DecValTok{1}\NormalTok{, }\AttributeTok{verbose =} \DecValTok{1}\NormalTok{,}
...
brings the analyses into a predictive framework Kristoufek (2015b) . The
key questions that can be answered by it are: a) \emph{How does the influence
of one time series on another time series change as a function of
scale?} and b) \emph{What is/are the dominant (time) scale(s) of influence of
